...
%% build_property/3: build_property(?Key, ?Value, ?Property)
...
%% read_edge_boundaries/3: read_edge_boundaries(+Edge, +Source_n, +Target_n)
...
%% read_property_content/3: read_property_content(+Property, -Key, -Value)
...
%% push_node/3: push_node(+Graph, +Node, -Result)
...
%%  push_edge/3: push_edge(+Graph, +Edge, -Result)
...
%% push_property/3: push_property(+Element, +Property, -Result)
...
%% push_property_list/3: push_property_list(+Element, +Property_list, -Result)
...
%% pull_node/3: pull_node(+Graph, +Node, -Result)
...
%% pull_edge/3: pull_edge(+Graph, +Edge, -Result)
...
%% pull_property/3: pull_property(+Element, +Property, -Result)
...
%% pull_from_db/2: pull_from_db(+Name, -Graph)
% Read a graph named Name in the fact database, and unify it wit Graph
pull_from_db(Name,Graph) :- 
	stored(Name, Nodes, Edges, Properties),
	Graph = gcoke_graph(Name, Nodes, Edges, Properties).

%% push_into_db/1: push_into_db(+Graph)
...
%% build_property_list/2: build_property_list(+Key_value_list, -Property_list)
...
%% build_key_value_list/2: build_key_value_list(+Property_list, -Key_value_list)
