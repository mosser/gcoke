...
This module implements usual queries executed on graphs
*/
%%%%
%% Scalar Getters (may be non-deterministic)
%%%%

%% get_node_by_name(+G, +Node_name, -Node)
...
%% get_edge_by_boundaries(+Graph, ?Source_name, ?Target_name, -Edge) is nondet
...
%% get_node_by_properties(+Graph, +Key_values, -Node) is nondet
...
%% get_edge_by_properties(+Graph, +Key_value_list, ?Source_n, ?Target_n, -Edge) is nondet
