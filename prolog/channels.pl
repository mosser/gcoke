...
:- module(channels, []).
/** <module> Message Broadacasting Channels
 ...
 How to know which channel to subscribe to?
   - gCoke artefacts may (its not mandatory) declare their channels using a 
     channels:declare(Channel) rules. However, it is simply informational.
...
unsubscribe(Channel) :- 
	gwf('%% Unsubscribing to channel %d\n',[Channel]),
	nodebug(Channel).
...
% the Format string uses ~-based data (instead of %-based for writef)
push(Channel, Format, Args) :- 
