...
%%%%
%% Rationale:
%%  this file allows one to sniff 'lines' in gCoKe.
%%
%% Line? What's the hell is that?
...
%%    you're in a Plain Old Prolog Source File. Load modules, others files, ...
%%  - Used paths are computed locally, that is, from your '_init.pl' file.
%%
%% How do I 'sniff' something?
%%  - You'll have to declare your personal lines repository (i.e., the directory
...
%%  - THIS (sniffs.pl) file CANNOT be defined in a module 'm'. In this case, 
%%    the loaded sniffs will be part of 'm' instead of the 'user' module.
%%%%
:- dynamic gcoke_sniff_repository/1.

%% gcoke_declare_repository/1: gcoke_declare_repository(+Directory)
...
%% gcoke_get_available_lines/1: gcoke_get_available_sniffs(-List)
...
%% [nondet] gcoke_is_available_line/1: gcoke_is_available_line(?Line_name)
...
%% gcoke_can_sniff/2: gcoke_can_sniff(+Line_name, -Path)
% Unify Path witg the concrete path of the '_init.pl' file associated to 
...
%% gcoke_sniff/1: gcoke_sniff(Line_name)
